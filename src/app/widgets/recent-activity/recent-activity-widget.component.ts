  ...
  template: `
    <div class="recent-activity-widget">
      ...
      @if (checkinStore.loading()) {
        <ff-loading-state text="Loading activity..." />
      } @else if (checkinStore.error()) {
        <ff-error-state [message]="checkinStore.error()!" />
        ...
        @if (hasMoreActivity()) {
          <button class="view-all-activity" (click)="navigateToFullActivity()" type="button">
            View All Activity
          </button>
        }
      }

...
@Component({
  ...
  template: `
    <section class="mission-detail-page">
      @if (isDataLoading()) {
        <ff-loading-state text="Loading mission details..." />
      } @else if (dataError()) {
        <ff-error-state
          [message]="dataError()!"
          [showRetry]="true"
          ...
          @if (userMissionData()) {
            <div class="progress-section">
              ...
              @if (userMissionData()!.isCompleted) {
                <div class="status-badge status-badge--completed">✅ Mission Completed!</div>
              } @else if (userMissionData()!.isActive) {
                <div class="status-badge status-badge--active">🔥 Mission Active</div>
              } @else {
                <app-button variant="primary" [size]="ButtonSize.LARGE" (onClick)="joinMission()">
                  🎯 Join Mission
                </app-button>
              }
            </div>
              ...
              @if (mission()!.requiredPubs) {
                <div class="detail-item">
                  ...
                  <span class="detail-value"
                    >{{ mission()!.requiredPubs }} of
                    {{ mission()!.totalPubs || mission()!.pubIds.length }}</span
                  >
                </div>
              }
...
export class MissionDetailComponent extends BaseComponent {
  ...
  // Route parameter
  protected readonly missionId = computed(

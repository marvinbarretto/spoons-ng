...
@Component({
  ...
  template: `
    <div class="admin-carpet">
      ...
      <!-- Loading State -->
      @if (loading()) {
        <app-loading-state text="Loading carpet data..." />
      } @else if (error()) {
        <app-error-state
          [message]="error()!"
          [showRetry]="true"
...
export class AdminCarpetComponent {
  ...
  readonly recentCarpetUploads = computed(() => {
    ...
    return this.carpetPubs().filter(p =>
      p.lastUpdated && p.lastUpdated > sevenDaysAgo
    ).length;
  });
  ...
  readonly averagePhotosPerDay = computed(() => {
    ...
    const daysDiff = Math.max(1, Math.ceil((now.getTime() - daysSinceFirst.getTime()) / (1000 * 60 * 60 * 24)));
  ...
  readonly totalStorageUsed = computed(() => {
    ...
    return estimatedMB > 1000 ? `${(estimatedMB / 1024).toFixed(1)}GB` : `${estimatedMB.toFixed(0)}MB`;
  });

  readonly weeklyUploads = computed(() => {
    const oneWeekAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
    return this.carpetPubs().filter(p =>
      p.lastUpdated && p.lastUpdated > oneWeekAgo
    ).length;
  });
  ...
  readonly monthlyUploads = computed(() => {
    ...
    return this.carpetPubs().filter(p =>
      p.lastUpdated && p.lastUpdated > oneMonthAgo
    ).length;
  });
  ...
  readonly peakUploadDay = computed(() => {
    ...
    const uploadsByDay = uploads.reduce((acc, carpet) => {
      const day = carpet.lastUpdated!.toLocaleDateString();
      acc[day] = (acc[day] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);

    const [peakDay, count] = Object.entries(uploadsByDay)
  ...
  readonly recentlyUpdatedCount = computed(() => {
    ...
    return this.carpetPubs().filter(p =>
      p.lastUpdated && p.lastUpdated > threeDaysAgo
    ).length;
  });

  readonly missingPhotosCount = computed(() => {
    return this.carpetPubs().filter(p =>
      p.carpetStatus === 'has-carpet' && !p.hasCloudImage
    ).length;
  });
  ...
  readonly citiesWithCarpets = computed(() => {
    const cityStats = this.getCityStats();
    const citiesWithCoverage = Object.values(cityStats)
      .filter(stats => stats.withCarpets > 0).length;
  ...
  private getCityStats() {
    return this.carpetPubs().reduce((acc, carpetPub) => {
  ...
  flagPhoto(pubId: string): void {
    const confirmed = confirm('Are you sure you want to flag this photo for review? This will mark it for moderation.');
    if (confirmed) {
      // TODO: Implement photo flagging functionality
      console.log('[AdminCarpet] Flagging photo for pub:', pubId);

      // For now, just show an alert
      alert('Photo has been flagged for review. In a full implementation, this would:\n\n' +
  ...
  onImageError(event: Event): void {
    ...
    img.src = 'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100"><rect width="100" height="100" fill="%23f0f0f0"/><text x="50" y="50" text-anchor="middle" dy=".3em" fill="%23999">No Image</text></svg>';
  }

...
export class CacheCoherenceService {
  ...
  constructor() {
    console.log(
      '🔄 [CacheCoherence] Service initialized - ready for cache invalidation management'
    );
  }
  ...
  wasRecentlyInvalidated(collection: string, withinMs: number = 5000): boolean {
    ...
    return this._invalidationHistory().some(
      inv => inv.collection === collection && inv.timestamp > cutoff
    );
  }
  ...
  private invalidateDependencies(
    ...
    dependencies.forEach((depCollection, index) => {
      setTimeout(
        ...
        (index + 1) * 75
      ); // Stagger dependency invalidations
    });
  }
  ...
  private addToHistory(invalidation: CacheInvalidation): void {
    ...
    const newStats: CacheStats = {
      ...
      collectionsInvalidated: new Set([
        ...currentStats.collectionsInvalidated,

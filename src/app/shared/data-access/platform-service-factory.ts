...
export class PlatformServiceFactory {
  ...
  private registerServices(): void {
    ...
    if (this.platformService.isNative()) {
      ...
      console.log(
        '[PlatformServiceFactory] 📱 Registered CapacitorLocationService:',
      ...
      console.log(
        '[PlatformServiceFactory] 🌐 Registered WebLocationService:',
        !!webLocationService
      );
  ...
  getService<T>(serviceKey: string): T {
    if (!this._initialized) {
      throw new Error(
        `PlatformServiceFactory not initialized. Service ${serviceKey} not available.`
      );
    }
    ...
    if (!service) {
      throw new Error(
        `Service ${serviceKey} not registered for platform ${this.platformService.platformName()}`
      );
    }

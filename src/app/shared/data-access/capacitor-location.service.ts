...
export class CapacitorLocationService extends AbstractLocationService {
    ...
    try {
      ...
      if (permissionStatus !== 'granted') {
        ...
        if (requestResult !== 'granted') {
          throw new Error(
            `Location permission ${requestResult}. Please enable location services in Settings.`
          );
        }
      }
  ...
  async requestPermission(): Promise<LocationPermissionStatus> {
    ...
    if (!this.platformService.isNative) {
      console.warn(
        '[CapacitorLocationService] Permission request only available on native platforms'
      );
  ...
  async initializeEarlyPermissionCheck(): Promise<boolean> {
    ...
    try {
      ...
      if (permissionStatus === 'granted') {
        console.log(
          '[CapacitorLocationService] ✅ Location permission already granted - ready for use'
        );
        return true;
      }
      ...
      if (this.canRequestLocation()) {
        console.log(
          '[CapacitorLocationService] 🟡 Location permission can be requested when needed'
        );
  ...
  async warmLocation(
    ...
    if (this.permissionStatus() !== 'granted') {
      console.log(
        '[CapacitorLocationService] 🔥 Skipping location warming - permission not granted'
      );
      return false;
    }
    ...
    if (!this.platformService.isNative) {
      console.log(
        '[CapacitorLocationService] 🔥 Skipping location warming - not on native platform'
      );
      return false;
    }
    ...
    try {
      console.log(
        '[CapacitorLocationService] 🔥 Calling Geolocation.getCurrentPosition for warming...'
      );
  ...
  async getCurrentLocationEnhanced(forceRefresh: boolean = false): Promise<LocationPosition> {
    console.log('[CapacitorLocationService] 📍 Enhanced location request...', {
      ...
      cachedLocationAge: this.currentPosition()?.timestamp
        ? Date.now() - this.currentPosition()!.timestamp!
        : 0,
    });
    ...
    if (!forceRefresh && this.currentPosition()) {
      ...
      if (cachedLocation.timestamp) {
        ...
        if (locationAge < 120000) {
          console.log(
            '[CapacitorLocationService] 📍 Using cached location (age: ' +

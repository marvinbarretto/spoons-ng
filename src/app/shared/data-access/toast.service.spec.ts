...
describe('ToastService', () => {
  ...
  describe('Adding Toasts', () => {
    ...
    it('should generate unique IDs for multiple toasts', () => {
      ...
      expect(toasts[0].id).toBe('id-2'); // Newest toast first
    ...
    it('should correctly set sticky property', () => {
      ...
      const toasts = service.toasts();
      expect(toasts[0].sticky).toBe(false); // Error toast (newest) was set to false
      expect(toasts[1].sticky).toBe(true);  // Success toast (oldest) was set to true
    });
  });

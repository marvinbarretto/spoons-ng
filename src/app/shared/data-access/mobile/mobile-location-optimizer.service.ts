...
export class MobileLocationOptimizer {
  ...
  async initializeMobileLocation(): Promise<void> {
    ...
    try {
      ...
      if (hasPermission) {
        ...
        console.log(`[${this.SERVICE_NAME}] 📍 Location permission not granted - will request when needed`);
      }
      ...
      console.log(`[${this.SERVICE_NAME}] ✅ Mobile location initialization complete (${duration}ms)`);
  ...
  private startLocationWarming(): void {
    ...
    this.capacitorLocationService.warmLocation({
      ...
      if (success) {
        ...
        console.log(`[${this.SERVICE_NAME}] ⚠️ Location warming failed - will retry on next app activation`);
      }
  ...
  async getLocationForCheckIn(maxCacheAge: number = 120000): Promise<LocationPosition> {
    ...
    try {
      ...
      if (this.capacitorLocationService.hasFreshCachedLocation(maxCacheAge)) {
        ...
        console.log(`[${this.SERVICE_NAME}] ⚡ Using cached location for check-in (age: ${Math.round(age/1000)}s)`);
        return cachedLocation;
      }

      // No fresh cached location - get new one with mobile optimizations
      console.log(`[${this.SERVICE_NAME}] 📍 No fresh cached location - requesting new location...`);
      ...
      const duration = Date.now() - startTime;
      console.error(`[${this.SERVICE_NAME}] 📍 Failed to get location for check-in (${duration}ms):`, error);
      throw error;
    }
  }

...
export class TestScenarioBuilder {
  ...
  withCheckInFrequency(
    frequency: TestScenarioConfig['checkInFrequency'],
    customRate?: number
  ): TestScenarioBuilder {
  ...
  private generateScenarioData(scenario: TestScenarioConfig): RelatedDataSet {
    ...
    console.log(
      `✅ Generated scenario with ${users.length} users, ${pubs.length} pubs, ${checkIns.length} check-ins`
    );
    ...
    return Array.from({ length: scenario.users }, (_, i) => {
      ...
      return {
        ...
        registrationDate: this.generateRegistrationDate(
          scenario.timespan!.start,
          scenario.timespan!.end
        ),
      };
    });
  }
  ...
  private generateCheckIns(scenario: TestScenarioConfig, users: any[], pubs: any[]): any[] {
    ...
    users.forEach(user => {
      const userCheckInCount = this.calculateUserCheckIns(
        user.userType,
        checkInRate,
        scenario.timespan!
      );
      const userCheckIns = this.generateUserCheckIns(
        user,
        pubs,
        userCheckInCount,
        scenario.timespan!
  ...
  private generateBadges(scenario: TestScenarioConfig): any[] {
    const badgeTemplates = [
      {
        name: 'First Timer',
        description: 'First pub check-in',
        category: 'milestone',
        rarity: 'common',
      },
      ...
      {
        name: 'Weekender',
        description: 'Check-in on weekends',
        category: 'social',
        rarity: 'uncommon',
      },
      {
    ...
    return badgeTemplates.map((template, i) =>
      createTestBadge({
        id: `badge-${i}`,
        name: template.name,
    ...
    const missionTemplates = [
      {
        ...
        description: 'Check-in 3 times today',
        type: 'daily',
        target: 3,
        points: 50,
      },
      {
        ...
        description: 'Check-in 5 times this weekend',
        type: 'weekend',
        target: 5,
        points: 100,
      },
      {
        ...
        description: 'Visit 2 new pubs this week',
        type: 'weekly',
        target: 2,
        points: 75,
      },
      {
        ...
        description: 'Check-in with friends 3 times',
        type: 'social',
        target: 3,
        points: 80,
      },
  ...
  private generateUserStats(users: any[], checkIns: any[], userBadges: any[]): any[] {
    return users.map(user => {
      ...
      return {
        ...
        totalPoints,
        badgeCount,
        averagePointsPerCheckIn:
          userCheckIns.length > 0 ? Math.round(totalPoints / userCheckIns.length) : 0,
        firstCheckIn: userCheckIns.length > 0 ? userCheckIns[0].timestamp : null,
        lastCheckIn:
          userCheckIns.length > 0 ? userCheckIns[userCheckIns.length - 1].timestamp : null,
  ...
  private getUserTypeProperties(userType: string): any {
    const typeProperties = {
      ...
      champion: {
        averageCheckInsPerWeek: 15,
        preferredTimeSlots: Array.from({ length: 12 }, (_, i) => i + 11),
      },
    };
  ...
  private calculateUserCheckIns(
    userType: string,
    baseRate: number,
    timespan: { start: Date; end: Date }
  ): number {
    const days = Math.ceil(
      (timespan.end.getTime() - timespan.start.getTime()) / (24 * 60 * 60 * 1000)
    );
  ...
  private generateCheckInTimestamp(
    timespan: { start: Date; end: Date },
  ...
  private calculateFavoriteDay(checkIns: any[]): string {
    ...
    const dayCounts = checkIns.reduce(
      (acc, c) => {
...
export class UserJourneySimulator {
   ...
   */
  simulateUserJourney(
    userType: 'casual' | 'regular' | 'power' | 'champion',
    ...
    const firstCheckInDate = new Date(
 ...
 */
export function generateIntegrationTestData(

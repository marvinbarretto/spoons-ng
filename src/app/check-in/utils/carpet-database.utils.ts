 ...
 */
export function getCarpetsByLocation(
  lat: number,
  lng: number,
  radiusKm: number = 0.1
): CarpetData[] {
...
export function getPatternThresholds(pattern: CarpetSignature['pattern']) {
  ...
  const avgBrightness =
    carpetsOfType.reduce((sum, c) => sum + c.signature.brightness, 0) / carpetsOfType.length;
  const avgComplexity =
    carpetsOfType.reduce((sum, c) => sum + c.signature.complexity, 0) / carpetsOfType.length;
  const avgTexture =
    carpetsOfType.reduce((sum, c) => sum + c.signature.textureScore, 0) / carpetsOfType.length;
  const avgContrast =
    carpetsOfType.reduce((sum, c) => sum + c.signature.contrast, 0) / carpetsOfType.length;

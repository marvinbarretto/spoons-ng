...
describe('CheckInStore - Business Critical Tests', () => {
  ...
  const testPubs: Pub[] = [
    ...
    createTestPub({
      id: 'home-pub-1',
      name: 'Home Pub',
      location: { latitude: 51.5076, longitude: -0.128 },
    }),
    createTestPub({
      id: 'manual-pub-1',
      name: 'Manual Pub',
      location: { latitude: 51.5077, longitude: -0.1281 },
    }),
  ];
  ...
  beforeEach(async () => {
    ...
    mockCarpetStrategy = {
      processCarpetCapture: vi
        .fn()
        .mockResolvedValue({ llmConfirmed: true, localKey: 'carpet-key' }),
    };

...
export class CarpetImageAnalysisService {
  ...
  async analyzeVideoFrame(videoElement: HTMLVideoElement): Promise<CarpetImageMetrics> {
    ...
    if (this.analysisCount > this.MAX_ANALYSIS_COUNT) {
      console.log(
        '[CarpetImageAnalysis] 🧹 Reached max analysis count, clearing state for memory management'
      );
      this.clearState();
    }
  ...
  private calculateRepetition(imageData: ImageData): number {
    ...
    for (const offset of offsets) {
      ...
      for (let y = 0; y < height; y += 8) {
        // Sample every 8th row
        for (let x = 0; x < width - offset; x += 8) {
          // Sample every 8th column
          ...
          if (difference < 30) {
            // Must be quite similar to count as pattern
  ...
  private calculateMotion(currentFrame: ImageData): { motionLevel: number; isStable: boolean } {
    ...
    for (let i = 0; i < current.length; i += 64) {
      ...
      const currentBrightness =
        0.299 * current[i] + 0.587 * current[i + 1] + 0.114 * current[i + 2];
      const previousBrightness =
        0.299 * previous[i] + 0.587 * previous[i + 1] + 0.114 * previous[i + 2];

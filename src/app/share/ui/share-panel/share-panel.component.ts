...
} from '../../data-access/social-media.service';
  ...
  template: `
    <section class="share-panel">
      <div class="share-actions">
        <app-button (click)="share()" [disabled]="!canShare()"> Share via... </app-button>

        <app-button (click)="copyLink()" variant="secondary">Copy Link </app-button>
      </div>

      <div class="social-buttons">
        <h3>Share on Social Media</h3>
        <div class="social-grid">
          <app-button (click)="shareToSocial('twitter')" variant="secondary" size="sm">
            Twitter/X
          </app-button>

          <app-button (click)="shareToSocial('facebook')" variant="secondary" size="sm">
            Facebook
          </app-button>

          <app-button (click)="shareToSocial('whatsapp')" variant="secondary" size="sm">
            WhatsApp
          </app-button>

          <app-button (click)="shareToSocial('telegram')" variant="secondary" size="sm">
            Telegram
          </app-button>
        </div>
      </div>
...
export class SharePanelComponent extends BaseComponent {
  ...
  protected readonly canShare = computed(() => this._platform.isBrowser && !!navigator.share);

  share(): void {
    this._platform.onlyOnBrowser(() => this._share.shareApp(this.shareUrl()));
  }
